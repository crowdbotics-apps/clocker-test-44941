...
function postToSlackBuild(appInfo, buildInfo) {
    const message = `The status of build version *${buildInfo.version_string}* for your app *${appInfo.name}* has been changed to *${buildInfo.app_store_state.replace(/_/g, ' ').trim()}*`
...
function slackAttachmentBuild(fallback, appInfo, buildInfo) {
    const attachment = {
        fallback,
        "color": colorForStatus(buildInfo.app_store_state),
        "title": "App Store Connect",
        "author_name": appInfo.name,
        "title_link": `https://appstoreconnect.apple.com/apps/${appInfo.id}/appstore`,
        "fields": [
            {
                "title": "Build Version",
                "value": buildInfo.version_string,
                "short": true
            },
            {
                "title": "Build Status",
                "value": buildInfo.app_store_state,
                "short": true
            }
        ],
        "footer": "App Store Connect",
